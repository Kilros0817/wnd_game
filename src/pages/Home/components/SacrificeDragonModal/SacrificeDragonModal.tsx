    ...
    >
      <TokenContainer>
        {tokenDragons.map((item: any) => (
          <TokenWrapper
            key={item.id}
            onClick={() => handleSelectDragon(item.id)}
            className={selectedDragon.includes(item.id) ? 'selected' : ''}
          >
      ...
      <StyledButton
        type="primary"
        onClick={() => {
          if (dragonsLeftToSacrifice === 0) message.error('Nothing left');
          else if (selectedDragon.length === 0) message.error('Please select your NFTs to Sacrifice');
...
export default memo(SacrificeDragonModal);